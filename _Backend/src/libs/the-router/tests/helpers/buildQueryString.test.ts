...
describe('buildQueryString', () => {
  ...
  test('should handle string parameters', () => {
    expect(buildQueryString({ name: 'John', city: 'New York' })).toBe('?name=John&city=New+York');
  });
  ...
  test('should handle method parameter', () => {
    expect(buildQueryString(undefined, '_method=delete')).toBe('?_method=delete');
  });

  test('should combine method parameter with other parameters', () => {
    expect(buildQueryString({ force: true, reason: 'test' }, '_method=delete')).toBe(
      '?_method=delete&force=true&reason=test',
    );
  });
  ...
  test('should handle complete complex URLs with method parameter', () => {
    ...
    expect(buildQueryString({ delimiter: ';' }, '_method=get')).toBe('?_method=get&delimiter=%3B');
    ...
    ).toBe('?_method=post&status=in+progress&priority=high%21&labels=bug%2Cfeature');
  });
});
